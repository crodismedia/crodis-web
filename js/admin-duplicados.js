(function(){
  ...
  function construirCandidatos(talleres){
    ...
    talleres.forEach(t=>{
      ...
      agregarGrupo(porTelefono,tel,t);
      if(nom&&ciu)agregarGrupo(porNombreCiudad,`${nom}|${ciu}`,t);
      if(nom&&cp)agregarGrupo(porNombreCp,`${nom}|${cp}`,t);
    });
    const acum=new Map();
    ...
    return [...acum.values()].map(x=>{
      const s=[...x.senales];
      let confianza="normal",peso=1;
      if(s.includes("Mismo teléfono")&&s.length>=2){confianza="alta";peso=3;}
  ...
  function renderTaller(t){
    return `<strong>${esc(t.nombre||"Sin nombre")}</strong><small>${esc(t.ciudad||"—")} · ${esc(t.provincia||"—")} · ${esc(t.codigo_postal||"—")}</small><small>${esc(t.telefono||"Sin teléfono")} · ID ${esc(String(t.id).slice(0,12))}</small>`;
  }

  function render(){
    const inicio=pagina*PAGE_SIZE,fin=inicio+PAGE_SIZE,filas=visibles.slice(inicio,fin);
    $("tabla-duplicados").innerHTML=filas.length?filas.map(c=>`<tr>
      <td><span class="chip ${c.confianza}">${c.confianza[0].toUpperCase()+c.confianza.slice(1)}</span></td>
      <td>${renderTaller(c.a)}</td>
      <td>${renderTaller(c.b)}</td>
      <td><div class="signals">${c.senales.map(s=>`<span class="chip">${esc(s)}</span>`).join("")}</div></td>
      <td><div class="actions"><a href="/talleres/${encodeURIComponent(slug(c.a))}" target="_blank" rel="noopener">Ficha A</a><a href="/talleres/${encodeURIComponent(slug(c.b))}" target="_blank" rel="noopener">Ficha B</a><a href="admin-talleres.html">Gestión</a></div></td>
    </tr>`).join(""):'<tr><td colspan="5" style="padding:28px;text-align:center;color:#66736b">No hay candidatos con estos filtros.</td></tr>';
    ...
    $("dup-anterior").disabled=pagina<=0;
    $("dup-siguiente").disabled=fin>=visibles.length;
  }

  function filtrar(reset=true){
    if(reset)pagina=0;
    const q=norm($("dup-busqueda").value);
    const confianza=$("dup-confianza").value;
    visibles=candidatos.filter(c=>{
      if(confianza&&c.confianza!==confianza)return false;
      if(!q)return true;
      const texto=norm([c.a.nombre,c.a.ciudad,c.a.telefono,c.a.id,c.b.nombre,c.b.ciudad,c.b.telefono,c.b.id,c.senales.join(" ")].join(" "));
  ...
  function enlazar(){
    ...
    $("btn-filtrar").addEventListener("click",()=>filtrar(true));
    $("dup-busqueda").addEventListener("keydown",e=>{if(e.key==="Enter"){e.preventDefault();filtrar(true);}});
    $("dup-confianza").addEventListener("change",()=>filtrar(true));
    $("dup-anterior").addEventListener("click",()=>{if(pagina>0){pagina--;render();}});
    $("dup-siguiente").addEventListener("click",()=>{if((pagina+1)*PAGE_SIZE<visibles.length){pagina++;render();}});
