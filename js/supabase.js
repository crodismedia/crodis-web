(function () {
    ...
    function actualizarNumeroResultados(total) {
        const cantidad = Number.isFinite(Number(total))
    ...
    async function adjuntarFotosFirmadas(talleres) {
        ...
        return talleres.map(taller => {
            const ruta = Array.isArray(taller.fotos)
                ? taller.fotos[0]
                : "";

            return {
                ...taller,
                fotoFirmada: porRuta.get(ruta) || ""
            };
        });
    }
    ...
    async function cargarServicios() {
        ...
        (data || []).forEach(servicio => {
            ...
            opcion.textContent =
                servicio.nombre
                || etiquetaDesdeSlug(servicio.slug);
        ...
        if (
            [...selector.options].some(
                opcion => opcion.value === seleccionado
            )
        ) {
        ...
        if (
            [...selector.options].some(
                opcion => opcion.value === servicioUrl
            )
        ) {
        ...
        const escribir = (id, valor) => {
            ...
            if (elemento && Number.isFinite(numero)) {
                elemento.textContent =
                    new Intl.NumberFormat("es-ES").format(numero);
            }
        };

        escribir(
            "contador-altas-cabecera",
            data?.talleres_activos
        );

        escribir(
            "estadistica-talleres",
            data?.talleres_activos
        );
    ...
    async function ejecutarBusquedaProfesional({
        ubicacion,
        servicio,
        desde,
        limite
    ...
    async function cargarTalleres(
        ...
        try {
            const { data, error } =
                await ejecutarBusquedaProfesional({
            ...
            const talleres = Array.isArray(data)
                ? data
                : [];
            ...
            if (!talleres.length) {
                if (reiniciar) {
                    mostrarEstado(
                        ...
                        "Prueba otra población, código postal o servicio."
                    );
            ...
            const total = Number(
                talleres[0]?.total_resultados
            );

            totalResultadosActual = Number.isFinite(total)
                ? total
                : siguienteIndice;

            actualizarNumeroResultados(
                totalResultadosActual
            );
            ...
            console.error(
                "Error inesperado durante la búsqueda:",
                error
            );

            if (reiniciar) {
                mostrarEstado(
                    "Ha ocurrido un problema al buscar talleres. " +
                    "Vuelve a intentarlo."
                );
            }
    ...
    function iniciarFormulario() {
        ...
        const poblacion = document.getElementById(
            "poblacion"
        );
        ...
        const cargarMas = document.getElementById(
            "boton-cargar-mas"
        );
        ...
        document
            .querySelectorAll("[data-servicio]")
            .forEach(enlace => {
    ...
    function cargarBusquedaInicialDesdeUrl() {
        const poblacion = document.getElementById(
            "poblacion"
        );
        ...
        const poblacionUrl = terminoSeguro(
            parametros.get("poblacion") || ""
        );

        const servicioUrl = terminoSeguro(
