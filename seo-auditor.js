...
class SEOAuditor {
  ...
  analyzeKeywords(html) {
    ...
    const coreKeywords = CONFIG.keywords.slice(0, 7); const missingKeywords = coreKeywords.filter(keyword => keywordAnalysis[keyword]?.status === 'missing'); if (missingKeywords.length > 0) { this.addIssue('keywords', `Faltan palabras clave núcleo: ${missingKeywords.slice(0, 3).join(', ')}`, 'warning', 'keywords'); this.addRecommendation(`Incluir de forma natural palabras clave núcleo: ${missingKeywords.slice(0, 3).join(', ')}`, 'medium'); }
