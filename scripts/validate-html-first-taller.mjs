...
requireCondition(
  (routeConfig.rewrites || []).some(
    (rewrite) =>
      rewrite?.source === "/talleres/:slug" &&
      rewrite?.destination === "/api/taller-html?slug=:slug"
  ),
  "La ruta pública de talleres debe usar el render HTML-first."
);
...
requireCondition(
  renderer.includes("function stripConflictingRuntime") &&
  renderer.includes("return stripConflictingRuntime(html)"),
  "El render debe retirar los runtimes que reconstruían el contenido en cliente."
);
...
requireCondition(
  renderer.includes("ficha-publica-portada-verde") &&
  renderer.includes("Horario de atención"),
  "La ficha pública debe conservar la portada verde con el horario."
);
...
requireCondition(template.includes('data-tallermap-supabase-aux="true"'), "Las funciones auxiliares que consultan Supabase deben conservar su SDK cliente.");
