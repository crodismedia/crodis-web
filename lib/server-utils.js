...
export function reviewStatusLabel(_verified) {
    return "";
}
...
export function renderWorkshopMedia(_workshop, _name) {
    return "";
}
...
export async function enrichWorkshopServices(workshops) {
    const rows = Array.isArray(workshops) ? workshops : [];
    const ids = [...new Set(rows.map((row) => String(row?.id || "").trim()).filter((id) => /^[0-9a-f-]{36}$/i.test(id)))];
    if (!ids.length) return rows;

    const url = new URL(`${SUPABASE_URL}/rest/v1/talleres`);
    url.searchParams.set("select", "id,servicios");
    url.searchParams.set("id", `in.(${ids.join(",")})`);
    url.searchParams.set("limit", String(ids.length));

    const result = await fetch(url, { headers: { apikey: SUPABASE_KEY } });
    if (!result.ok) {
        const message = await result.text().catch(() => "");
        throw new Error(`servicios de talleres respondió ${result.status}: ${message.slice(0, 300)}`);
    }

    const servicesById = new Map((await result.json()).map((row) => [String(row.id), Array.isArray(row.servicios) ? row.servicios : []]));
    return rows.map((row) => ({ ...row, servicios: servicesById.get(String(row.id)) || [] }));
}
