...
export function renderWorkshopMedia(workshop, name, badge = "") {
    ...
    const image = source.url
        ...
        : "";
    const badgeHTML = badge ? `<span class="verificado">${escapeHTML(badge)}</span>` : "";
    return `<div class="taller-imagen ${source.url ? "taller-imagen-real" : "taller-imagen-1"}"${pathAttribute}>${image}${badgeHTML}</div>`;
}
