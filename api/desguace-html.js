...
export default async function handler(req, res) {
  ...
  if (!/^[a-z0-9-]{2,160}$/.test(slug)) {
    res.status(404).setHeader('Content-Type','text/html; charset=utf-8').send('<h1>404</h1>');
    return;
  }
  try {
    const d = await fetchDesguace(slug);
    if (!d) {
      res.status(404).setHeader('Content-Type','text/html; charset=utf-8').send('<h1>Desguace no encontrado</h1>');
      return;
    }
