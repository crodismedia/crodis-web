...
function renderWorkshop(workshop, index) {
    ...
    const address = [workshop.direccion, workshop.codigo_postal, workshop.ciudad, workshop.provincia]
        ...
        .map(escapeHTML)
        .join(", ");
    const phone = String(workshop.telefono || "").replace(/[^\d+]/g, "").slice(0, 20);
    ...
    return `
        <article class="taller-card" data-taller-index="${index}" data-taller-slug="${escapeHTML(slug)}">
            ...
            <div class="taller-informacion">
                <h3>${slug ? `<a class="enlace-ficha-taller" href="/talleres/${encodeURIComponent(slug)}">${name}</a>` : name}</h3>
                <p class="ubicacion">⌖ ${address || "Ubicación no indicada"}</p>
                <p class="taller-descripcion">${description}</p>
                <div class="especialidades">${serviceHTML}</div>
                ${renderSchedule(workshop.horarios)}
                <div class="taller-pie">
                    <span class="taller-contactos">${contacts.join("") || "Sin contacto publicado"}</span>
                </div>
            </div>
        </article>
    `;
...
function injectPagination(html, fileName, page, total, service) {
    ...
    const pagination = total > PAGE_SIZE ? `
        <div id="contenedor-cargar-mas" class="cargar-mas-contenedor municipio-paginacion">
            <a id="boton-pagina-anterior" class="boton boton-claro${previousDisabled ? " deshabilitado" : ""}" aria-disabled="${previousDisabled}" href="${escapeHTML(pageURL(fileName, Math.max(1, page - 1), service))}">← Anterior</a>
            <span id="estado-paginacion" aria-live="polite">Página ${page} de ${totalPages}</span>
            <a id="boton-pagina-siguiente" class="boton${nextDisabled ? " deshabilitado" : ""}" aria-disabled="${nextDisabled}" href="${escapeHTML(pageURL(fileName, Math.min(totalPages, page + 1), service))}">Siguiente →</a>
        </div>` : `<div id="contenedor-cargar-mas" class="cargar-mas-contenedor" hidden></div>`;

    return html.replace(
        /<div id="contenedor-cargar-mas" class="cargar-mas-contenedor" hidden>[\s\S]*?<\/div>/i,
        pagination
    );
}
...
function injectHeadSEO(html, fileName, page, total, service) {
    ...
    if (indexable && page > 1) {
        ...
        result = result.replace(
            ...
            `<link rel="canonical" href="https://www.tallermap.es${pageURL(fileName, page, "")}">`
        );
    }
...
export default async function handler(request, response) {
    ...
    let municipality;
    try {
        municipality = readMunicipalityData(html);
    } catch (error) {
        console.error("No se pudo leer la plantilla municipal:", error);
        response.status(500).send("No se pudo renderizar el municipio.");
        return;
    }

    try {
        const from = (page - 1) * PAGE_SIZE;
        const workshops = await rpc("buscar_talleres_municipio", {
        ...
        html = html.replace(
            /<span class="orden-talleres mapa-estado"[^>]*>[\s\S]*?<\/span>/i,
